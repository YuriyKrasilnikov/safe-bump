...
/// Iterator over items in a [`SharedArena`] in allocation order.
///
/// Created by [`SharedArena::iter`].
...
impl<'a, T> Iterator for SharedArenaIter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.pos < self.len {
            let val = self.storage.get(self.pos);
            self.pos += 1;
            Some(val)
        } else {
            None
...
/// Iterator yielding `(Idx<T>, &T)` pairs from a [`SharedArena`]
/// in allocation order.
///
/// Created by [`SharedArena::iter_indexed`].

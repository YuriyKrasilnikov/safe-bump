...
/// Builds several archived segments by repeatedly rolling back to a fresh
/// checkpoint whose length exceeds the *start* of the segment just
/// discarded — exactly how `Identity`'s archive table grows one entry per
/// call (see `crate::segments::Identity::commit`'s `prefix_tail` branches:
/// the O(1) "growing" branch is what appends to the table). Cross-checks
/// `is_valid`, through the public surface only, against a brute-force
/// oracle built independently of the arena's own bookkeeping: a handle is
/// expected valid iff its slot is still below the current length and no
/// later rollback ever discarded that slot.
#[test]
fn is_valid_agrees_with_a_brute_force_oracle_across_many_increasing_rollbacks() {
    fn alloc_at(
        arena: &mut Arena<usize>,
    ...
    for _ in 0..6 {
        alloc_at(&mut arena, &mut slot_epoch, &mut handles, epoch);
    }
    ...
    // archives nothing (the `Identity::commit` branch that leaves
...
/// one slot `Identity::commit`'s `prefix_tail` cache exists for (see
/// `crate::segments`): `checkpoint.len() - 1 == current_start - 1`, right
/// after `current_start` was just set. This must still round-trip through
/// `try_rollback`/`validate_checkpoint`, and once the prefix it names is
/// later discarded and rebuilt under a fresh generation, the same
/// checkpoint must be rejected as `DivergedPrefix` rather than wrongly
/// accepted.
#[test]
...
/// `None` at `len == 0`, so it never reads `prefix_tail` at all — this
/// exercises the branch where `Identity::commit` leaves `prefix_tail`
/// untouched.
#[test]
